...
**For `κ = 2` (Erdős #942 itself), this lower bound is formalized and
machine-checked in Lean 4 / Mathlib** — see `powerful_count_rate` below.
...
`κ ≥ 2`. The `κ = 2` case is formalized in full (next section).
...
**1. The rate (`Erdos942/Rate.lean`).** The full `κ = 2` lower bound — the
quantitative rate, not merely unboundedness:

| Theorem | Statement |
|---|---|
| `powerful_count_rate` | there is `c > 0` such that for infinitely many `n`, at least `c · log n / (log log n · log log log n)` powerful numbers lie in `(n², (n+1)²)` |

Supporting lemmas in the same module: `nth_prime_upper` (the `h`-th prime is
`O(h log h)`, via `Chebyshev.pi_ge`), `box_principle_quantitative` (simultaneous
Dirichlet with the denominator bound), `squarefree_many_divisors` (a primorial
with `h` prime factors has `2^h − 1` squarefree divisors `> 1`), `log_primorial_le`
